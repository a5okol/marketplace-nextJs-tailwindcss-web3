...
const toBN = (value) => web3.utils.toBN(value); // to big numbers
...
contract('CourseMarketplace', (accounts) => {
  const courseId = '0x00000000000000000000000000003130'; // 16 bytes
  const proof =
    '0x0000000000000000000000000000313000000000000000000000000000003130'; // 32 bytes
  ...
  describe('Repurchase course', () => {
    ...
    it('should be able repurchase with the original buyer', async () => {
      ...
      assert.equal(
        toBN(beforeTxBuyerBalance).sub(toBN(value)).sub(gas).toString(), // sub (substructions) unlocking if you adre using toBN
        afterTxBuyerBalance,
